...
if __name__ == "__main__":
    ...
    if size > 0:
        with open(options.input, 'r') as input:
            nonCRs = {}
            averageDepths = {}
            results = {cutoff: [] for cutoff in A_cutoffs}
        ...
        for cutoff in A_cutoffs:
            with open(options.output + "." + cutoff + ".txt", 'w'):
                pass
        sys.stderr.write("No formatted sites; wrote empty candidate files successfully.\n")
